...
def main() -> None:
    ap=argparse.ArgumentParser(); ap.add_argument("--data",type=Path,default=Path("models/a100_training/product_nli/all.jsonl")); ap.add_argument("--out-dir",type=Path,default=Path("models/a100_training/product_nli/model")); ap.add_argument("--model-name",default="hfl/chinese-macbert-base"); ap.add_argument("--epochs",type=float,default=3); ap.add_argument("--batch-size",type=int,default=32); ap.add_argument("--max-length",type=int,default=384); ap.add_argument("--seed",type=int,default=20260715); args=ap.parse_args()
    try:
    ...
    trainer.train(); result=trainer.evaluate(); args.out_dir.mkdir(parents=True,exist_ok=True); trainer.save_model(str(args.out_dir/"best_model")); tokenizer.save_pretrained(str(args.out_dir/"best_model")); config={"data":str(args.data),"model_name":args.model_name,"epochs":args.epochs,"batch_size":args.batch_size,"max_length":args.max_length,"seed":args.seed}; digest=hashlib.sha256((args.out_dir/"best_model"/"model.safetensors").read_bytes()).hexdigest() if (args.out_dir/"best_model"/"model.safetensors").exists() else None; (args.out_dir/"metrics.json").write_text(json.dumps({"labels":LABELS,"train":len(train),"dev":len(dev),"metrics":result,"config":config,"model_sha256":digest},ensure_ascii=False,indent=2),encoding="utf-8"); print(json.dumps(result,ensure_ascii=False,indent=2))
